...
DATA_SOURCE_REGISTRY = [
    DataSourceRegistry(
        ...
        source_note="普通国家/地区代码来源；不覆盖 EPO、EUIPO、PCT 等知识产权业务对象。",
    ),
    DataSourceRegistry(
        source_id="UN_M49",
    ...
    DataSourceRegistry(
        ...
        source_note="知识产权国家、其他实体和组织代码来源；EP、EM、WO 等以此核验。",
    ),
    DataSourceRegistry(
    ...
    DataSourceRegistry(
        source_id="BUSINESS_ECONOMIC_REGION_TAGS",
        source_name="Business and economic region tag references",
        source_type="authoritative_reference",
        source_owner="APEC / ASEAN / BRICS / EU public references and internal business policy",
        source_url="https://www.apec.org/; https://asean.org/; https://brics-russia2024.ru/; https://european-union.europa.eu/",
        source_version="Current public references; internal tag policy 2026-06",
        applicable_fields=("default_business_economic_regions",),
        verification_frequency="annual_or_internal_policy_change",
        source_note="仅作为商务/经济区域默认标签参考，不作为国际地理区域来源。",
    ),
]
...
def _country(
    ...
    currency: str,
    aliases: tuple[str, ...] = (),
    source_note: str = "普通国家/地区以 ISO 3166 代码、UN M49 地理区域和 WIPO ST.3 IP 代码交叉核验。",
    ...
    return JurisdictionReference(
        ...
        source_name="ISO 3166 / UN M49 / WIPO ST.3",
        source_url=f"https://www.iso.org/obp/ui/#iso:code:3166:{code}",
        source_version="ISO 3166 current; UN M49 current; WIPO ST.3 December 2025",
...
JURISDICTION_REFERENCES = [
    ...
    _country("BR", "巴西", "Brazil", "South America", ("LATIN_AMERICA", "SOUTH_AMERICA", "BRICS"), "BRL", ("BRA", "Brasil")),
